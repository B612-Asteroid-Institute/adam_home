...
class TestKeplerian:
    ...
    def _make_cartesian_and_keplerian_batches(self, start_time_str, end_time_str, working_project):
        keplerian_elements = {
    ...
    def test_keplerian_and_cartesian(self, service, working_project):
        start = datetime.datetime.now()
        end = start + datetime.timedelta(10)  # 10 days
        start_time_str = start.isoformat() + 'Z'
        end_time_str = end.isoformat() + 'Z'

        cartesian, keplerian = self._make_cartesian_and_keplerian_batches(
            start_time_str, end_time_str, working_project)

        BatchRunManager(service.get_batches_module(), [cartesian, keplerian]).run()
